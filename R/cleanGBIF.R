...
clean_gbif <- function(raw,
  ...
  dat_clean <- raw %>%
    ...
    filter(.data$occurrenceStatus  == "PRESENT") %>%

    filter(!.data$basisOfRecord %in% c("FOSSIL_SPECIMEN","LIVING_SPECIMEN")) %>%
    ...
    filter(!.data$decimalLatitude == 0 | !.data$decimalLongitude == 0) %>%
    ...
    # remove duplicates
    distinct(.data$decimalLongitude, .data$decimalLatitude,
             .data$speciesKey, .data$datasetKey, .keep_all = TRUE)  %>%


    filter(.data$decimalLongitude < 0)
